...
function inject (bot, options) {
  // 1.21.4+ (includes 26.x): notify server the client finished loading.
  // Gated on the real minecraft-data feature flag, not a version-string
  // check nor a bare try/catch — Serializer._transform reports an
  // unrecognized packet name via the stream's own async 'error' event
  // (protodef/src/serializer.js calls cb(e), not a synchronous throw), so
  // a bare try/catch around bot._client.write() never actually catches
  // that here; confirmed reading node-minecraft-protocol/protodef source
  // directly. mineflayer's own loader.js forwards bot._client 'error' to
  // bot.emit('error', ...), so an ungated write on any pre-1.21.4 server
  // would misfire as a spurious post-spawn connection failure.
